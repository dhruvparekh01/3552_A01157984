...
def setup_request_commandline() -> Request:
    """
    Implements the arg parse module to accept arguments via the command
...
class Crypto:
    ...
    def execute_request(self, request: Request):
        # import locally to stop circular imports
        from Labs.Lab9.handlers import ValidKeyHandler, FormatHandler, EncodingHandler, EncryptHandler, OutputHandler

        # instantiate handlers
        valid_handler = ValidKeyHandler()
        format_handler = FormatHandler()
        encoding_handler = EncodingHandler()
        encrypt_handler = EncryptHandler()
        output_handler = OutputHandler()

        # set up a handle chain
        valid_handler.set_handle(format_handler)
        format_handler.set_handle(encoding_handler)
        encoding_handler.set_handle(encrypt_handler)
        encrypt_handler.set_handle(output_handler)

        if request.encryption_state == CryptoMode.EN:
            self.encryption_start_handler = valid_handler
            self.encryption_start_handler.handle_request(request)
        else:
            self.decryption_start_handler = valid_handler
